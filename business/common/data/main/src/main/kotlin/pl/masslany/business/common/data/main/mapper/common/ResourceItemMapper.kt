...
class ResourceItemMapperImpl(
    ...
    override fun map(param: List<ResourceItemDto>): List<ResourceItem> {
        return param.map {
            ResourceItem(
                actions = it.actions?.let { actionsDto -> actionsMapper.map(actionsDto) }.toOption(),
                adult = it.adult ?: false,
                archive = it.archive ?: false,
                author = it.author?.let { authorDto -> authorMapper.map(authorDto) }.toOption(),
                comments = it.comments?.let { commentsDto -> commentsMapper.map(commentsDto) }.toOption(),
                ...
                hot = it.hot ?: false,
                id = it.id ?: -1,
                media = it.media?.let { mediaDto -> mediaMapper.map(mediaDto) }.toOption(),
                name = it.name.orEmpty(),
                publishedAt = it.publishedAt.toOption(),
                recommended = it.recommended ?: false,
                resource = resourceMapper.map(it.resource),
                slug = it.slug.orEmpty(),
                source = it.source?.let { source -> sourceMapper.map(source) }.toOption(),
                tags = it.tags ?: emptyList(),
                title = it.title.orEmpty(),
                voted = it.voted == 1,
                votes = it.votes?.let { votesDto -> votesMapper.map(votesDto) }.toOption(),
            )
        }
    }
}
